...
// Good: One line is acceptable if the next statement is not complex and only one line
if ($condition) $this->compltetion();

// Good: When more than 1 function or multiple line statement use curly braces
if ($condition) {

    // group similar thigns together to make readability better

    $var = 1; 
    $invoice = $thie->get();

    $invoice->applyCoupom();
    $invoice->doTotalCheck();
...
// Bad : If only 1 line, then whitespace can be avoided
if ($foo) {
...
// Good: When more than 1 line statement use whitespace to group similar things together
if ($condition) {
...
```php
class PostsFavoriteController
class UsersImageController
class CompaniesLotController
{
    public function favorite(Post $post)
    {
        request()->user()->favorites()->attach($post);
